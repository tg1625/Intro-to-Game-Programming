...
void Initialize() {
    ...
    state.platforms[10].position = glm::vec3(1.7f, 0.75f, 0); //floating right
    state.platforms[11].textureID = platformTextureID; 
    state.platforms[11].position = glm::vec3(-1.5f, -1.0f, 0); //floating left
 
 
    for (int i = 0; i < PLATFORM_COUNT; i++) {
        state.platforms[i].Update(0, NULL, 0);
        state.platforms[i].entityType = EntityType::GROUND;
    }
